...
var nodes = dom.empty(window.empty); //找到html中empty元素，删掉，然后把它所有儿子都弄出来

console.log(nodes); //然后打印出所有儿子

dom.attr(test, 'title', 'Hi, I am dw'); //参数长度为3，设置。写

var title = dom.attr(test, 'title'); //参数长度为2,读。获取test元素的title属性，并赋给变量title。读

console.log("title: ".concat(title)); //title值为title
...
console.log(dom.parent(test)); //console.log(dom.siblings(dom.find('#s2')[0]))//找到e2，再获取它的兄弟节点

var s2 = dom.find('#s2')[0];
console.log(dom.siblings(s2));
console.log(dom.next(s2));
var t = dom.find('#travel')[0]; //find返回的是一个数组，找到第一个元素
